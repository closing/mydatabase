...
<html>
  ...
  %>
      <font color="red" size="+2">您删除了一本书：<em><%=convert(book.getTitle())%></em><br>&nbsp;<br></font>
  <% 
    }
    if (request.getParameter("Clear") != null) {
      cart.clear();
  %>
    <font color="red" size="+2"><strong>清空购物车</strong><br>&nbsp;<br></font>
  <%
    }
  ...
  %>
	  ...
	  <td bgcolor="#ffffaa"><strong><a href="<%=request.getContextPath()%>/bookdetails.jsp?bookId=<%=book.getBookId()%>"><%=convert(book.getTitle())%></a></strong></td>
	  <td bgcolor="#ffffaa" align="right"><%=book.getPrice()%></td>
	  <td bgcolor="#ffffaa"><strong><a href="<%=request.getContextPath()%>/showcart.jsp?Remove=<%=book.getBookId()%>">删除</a></strong></td>
	</tr>
  <%
      }
  %>
        <tr><td colspan="4" bgcolor="#ffffff"><br></td></tr>
	<tr><td colspan="3" align="right" bgcolor="#ffffff">总额(元)</td>
